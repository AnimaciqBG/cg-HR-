...
export default function EmployeeProfile() {
  ...
  if (!employee) {
    return <div className="text-center py-12"><p className="text-gray-500">Employee not found</p></div>;
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <Link to="/employees" className="p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700">
      ...
      <div className="card p-6">
        <div className="flex items-start gap-6">
          <div className="w-20 h-20 rounded-2xl bg-primary-100 dark:bg-primary-900 flex items-center justify-center flex-shrink-0">
            {employee.photoUrl ? (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="card p-6">
          ...
          {editing ? (
            ...
            <div className="space-y-3">
              {employee.user?.email && (
                <div className="flex items-center gap-2"><Mail className="w-4 h-4 text-gray-400" /> <span className="text-sm">{employee.user.email}</span></div>
              )}
              {employee.phone && (
                <div className="flex items-center gap-2"><Phone className="w-4 h-4 text-gray-400" /> <span className="text-sm">{employee.phone}</span></div>
              )}
              {employee.address && (
                <div className="flex items-center gap-2"><MapPin className="w-4 h-4 text-gray-400" /> <span className="text-sm">{employee.address}</span></div>
              )}
            </div>
          )}
        </div>
        ...
        <div className="card p-6">
          ...
          <dl className="space-y-3 text-sm">
            <div className="flex justify-between"><dt className="text-gray-500">Contract</dt><dd className="font-medium">{employee.contractType}</dd></div>
            <div className="flex justify-between"><dt className="text-gray-500">Weekly Hours</dt><dd className="font-medium">{employee.weeklyHours}h</dd></div>
            {employee.manager && (
              <div className="flex justify-between"><dt className="text-gray-500">Manager</dt><dd className="font-medium">{employee.manager.firstName} {employee.manager.lastName}</dd></div>
            )}
            {employee.probationEndDate && (
              <div className="flex justify-between"><dt className="text-gray-500">Probation Ends</dt><dd className="font-medium">{new Date(employee.probationEndDate).toLocaleDateString('bg-BG')}</dd></div>
            )}
            <div className="flex justify-between"><dt className="text-gray-500">Role</dt><dd><span className="badge badge-blue">{employee.user?.role}</span></dd></div>
          </dl>
        </div>
        ...
        {employee.subordinates && employee.subordinates.length > 0 && (
          <div className="card p-6 lg:col-span-2">
            <h3 className="font-semibold mb-4">Direct Reports ({employee.subordinates.length})</h3>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {employee.subordinates.map((sub) => (
                <Link key={sub.id} to={`/employees/${sub.id}`} className="p-3 rounded-lg border dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700">
